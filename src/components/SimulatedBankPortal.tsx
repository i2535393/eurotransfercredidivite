...
export default function SimulatedBankPortal({ 
  ...
  const sendEmailAlert = (type: 'SUCCESS' | 'FAILURE', details?: any) => {
    ...
    const feesVal = vamt * (transfer.feePercent / 100);
    const netVal = vamt - feesVal;
    
    let subject = '';
    let body = '';
    
    if (type === 'SUCCESS') {
      subject = `✅ [TRANSFERWIRE] Virement de ${vamt.toLocaleString('fr-FR')} ${curCode} effectué avec succès`;
      body = `
          ...
          <div style="padding: 24px; font-size: 14px; line-height: 1.6; color: #334155;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin: 20px 0; background-color: #f8fafc; border-radius: 8px;">
              ...
              <tr>
                ...
                <td style="padding: 10px 14px; border-bottom: 1px solid #f1f5f9; font-weight: bold; text-align: right; color: #0f172a; font-size: 16px;">${vamt.toLocaleString('fr-FR')} ${curSymbol}</td>
              </tr>
              <tr>
      ...
      body = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #fecdd3; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
          ...
          <div style="padding: 24px; font-size: 14px; line-height: 1.6; color: #334155;">
            ...
            <p>Notre automate interbancaire de surveillance de routage a immédiatement bloqué le virement externe sortant de <strong>${vamt.toLocaleString('fr-FR')} ${curSymbol}</strong> initié depuis votre compte.</p>
            ...
            <table style="width: 100%; border-collapse: collapse; margin: 15px 0; background-color: #fffaf0; border: 1px solid #fef3c7;">
              <tr>
                ...
                <td style="padding: 10px 14px; border-bottom: 1px solid #fef3c7; font-weight: bold; text-align: right; color: #b45309;">${vamt.toLocaleString('fr-FR')} ${curSymbol}</td>
              </tr>
    ...
    if (onTriggerEmailNotification) {
      onTriggerEmailNotification(
        ...
        `Notification envoyée à ${transfer.email} pour le virement d'un montant de ${vamt} EUR.`,
                          ...
                          <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-sans">
                              <div className="space-y-1">
                                ...
                                <strong className="text-sm text-slate-900 font-black">{(virementAmount || parseFloat(virementAmountInput) || transfer.amount).toLocaleString('fr-FR')} {curSymbol}</strong>
                              </div>
                              <div className="space-y-1">
            ...
            <div className="w-full max-w-md bg-white rounded-3xl overflow-hidden border border-slate-300 shadow-2xl animate-scale-up text-left">
              ...
              <div className="p-5 sm:p-6 space-y-4">
                ...
                <div className="bg-slate-50 border rounded-2xl p-4 space-y-2 text-xs text-slate-650 divide-y divide-slate-101">
                  ...
                  <div className="flex justify-between py-1 pt-2 font-sans select-all">
                    <span>Montant à recevoir :</span>
                    <strong className="text-slate-900 text-sm font-black">{(virementAmount || parseFloat(virementAmountInput) || transfer.amount).toLocaleString('fr-FR')} €</strong>
                  </div>
                </div>
        ...
        {showSuccessModal && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-white rounded-3xl overflow-hidden border border-slate-300 shadow-2xl animate-scale-up text-left">
              ...
              <div className="p-5 sm:p-6 space-y-4">
                <div className="bg-emerald-50 border border-emerald-150 p-3.5 rounded-2xl text-emerald-800 text-xs leading-relaxed font-semibold">
                  Le routage interbancaire complété à 100% de compensation. Les fonds d'un montant net de <strong>{(virementAmount || parseFloat(virementAmountInput) || transfer.amount).toLocaleString('fr-FR')} €</strong> ont été approuvés par l'habilitation de sécurité.
                </div>
